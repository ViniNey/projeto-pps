...
module.exports = ({
  ...
  router.get("/admin/me", verificarToken, (req, res) => {
    res.json({
      id: req.user.id,
      role: req.user.role,
  ...
  router.get("/me/produtos", verificarToken, (req, res) => {
  ...
  router.post(
    ...
    async (req, res) => {
      try {
        ...
        const imagemUrl = req.body.imagem || null; 
        const imagemPublicId = null;
  ...
  router.post("/pedidos/salvar", (req, res) => {
    const {
      slug,
      itens,
      forma_pagamento,
    ...
    connection.query(
      ...
      (err, estabResult) => {
        ...
        connection.query(
          `INSERT INTO pedidos 
          (establishment_id, itens, total, forma_pagamento, tipo_entrega, mesa, nome_cliente, endereco)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
          [
            establishment_id,
          ...
          (insertErr, resultPedido) => {
            if (insertErr) {
              console.error(insertErr);
              return res.status(500).json({ mensagem: "Erro ao salvar pedido" });
            }
  ...
  router.get("/admin/pedidos", verificarToken, (req, res) => {
    const establishmentId = req.user.establishmentId;
  ...
  router.delete("/admin/pedidos/:id", verificarToken, (req, res) => {
    const { id } = req.params;
    ...
    connection.query(
      ...
      (err) => {
        if (err) {
          console.error(err);
          return res.status(500).json({ mensagem: "Erro ao excluir pedido" });
        }
  ...
  router.put("/admin/pedidos/:id/finalizar", verificarToken, (req, res) => {
